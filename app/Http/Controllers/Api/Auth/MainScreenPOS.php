...
class MainScreenPOS extends BaseController
{
    ...
    public function add_sale(Request $request)
    {
        ...
        $this->applyLoyaltyPoints($validatedData, $finalAmount);

        $orderData = $this->prepareOrderData($validatedData, $discount, $finalAmount);
        $order = Orders::create($orderData);
        $this->processOrderItems($order->id, $validatedData['items']);
    ...
    private function prepareOrderData($validatedData, $discount, $finalAmount)
    {
    ...
    private function processOrderItems($orderId, $items)
    {
        foreach ($items as $itemData) {
            ...
            $productDiscount = $this->fetchProductDiscount($foodMenu);
            ...
            $item->single_discount = $discountDetails['SingleDiscountAmount'] ?? 0;
            $item->menu_unit_price = $foodMenu->sale_price;
            $item->menu_price_with_discount = $foodMenu->sale_price - ($discountDetails['SingleDiscountAmount'] ?? 0);
            ...
            $item->menu_discount_value = $discountDetails['SingleDiscountAmount'] * $itemData['quantity'];
            $item->discount_type = $discountDetails['discount_type'] ?? 'none';
            $item->discount_amount = $discountDetails['sda'] ?? 0;
    ...
    private function fetchProductDiscount($foodMenu)
    {
        $currentDate = Carbon::now();
        return Discount::where('product_id', $foodMenu->id)
    ...
    private function calculateDiscount($foodMenu, $productDiscount)
    {
        ...
        if ($productDiscount) {
            ...
            if ($productDiscount->dis_type == "percentage") {
                $SingleDiscountAmount = ($foodMenu->price * $productDiscount->discount_amount) / 100;
